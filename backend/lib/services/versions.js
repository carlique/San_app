...
VersionsService.prototype.getAll = function (req, res, next) {
  Calculation.findById(req.params.calculationId).then(calculation => {
    if (!calculation) {
      req.log.info('VersionsService.getAll: calculation with calculationId not found: '+ req.params.calculationId);
      ...
      calculation.getVersions().then(versions => {
        req.log.info('VersionsService.getAll returns: '+ versions.length +' records');
...
VersionsService.prototype.getById = function (req, res, next) {
  Version.findById(req.params.id).then(version => {
    if (!version) {
      req.log.info('VersionsService.getById: id not found: '+ req.params.id);
      res.send(404, Response.error(null, "Couldn't find Version with id: " + req.params.id));
    } else {
      req.log.info('VersionsService.getById: ' + JSON.stringify(version));
...
VersionsService.prototype.create = function (req, res, next) {
  Calculation.findById(req.params.calculationId).then(calculation => {
    if (!calculation) {
      ...
      .then(version => {
        req.log.info('VersionsService.create: create with Id '+ version.id);
      ...
      .catch(Sequelize.ValidationError, function (err) {
        req.log.info('VersionsService.create: validation error');
...
VersionsService.prototype.update = function (req, res, next) {
  Calculation.findById(req.params.calculationId).then(calculation => {
    if (!calculation) {
      ...
      Version.findById(req.params.id).then(version => {
        if (!version) {
          req.log.info('VersionsService.update: id not found: '+ req.params.id);
          res.send(404, Response.error(null, "Couldn't find version with id: " + req.params.id));
          return next();
        }
        else {
          ...
          .then(version => {
            req.log.info('VersionsService Id:' + version.id + ' updated');
...
VersionsService.prototype.destroy = function (req, res, next) {
  Calculation.findById(req.params.calculationId).then(calculation => {
    if (!calculation) {
      ...
      Version.findById(req.params.id).then(version => {
        if (!version) {
          req.log.info('VersionsService.destroy: id not found: '+ req.params.id);
          res.send(404, Response.error(null, "Couldn't find version with id: " + req.params.id));
          return next ();
        }
        else {
          version.destroy()
          .then(function() {
            req.log.info('VersionsService version with id: ' + req.params.id + ' deleted');
